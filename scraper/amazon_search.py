...
class AmazonSearchScraper:
    ...
    Block/challenge retries use a fresh browser context when the owner provides
    a context reset callback. Ordinary navigation errors only replace the page.
    """
    ...
    def _close_and_replace_page(self):
        """Replace only the page after an ordinary navigation failure."""
        old_page = self.page
        context = None
        try:
    ...
    def _reset_context(self):
        """Replace the entire browser context after a block/challenge."""
    ...
    def _navigate(self, url: str, category_name: str = ""):
        self.page.set_default_navigation_timeout(self.navigation_timeout_ms)
        try:
            return self.page.goto(url, wait_until="domcontentloaded", timeout=self.navigation_timeout_ms)
        except Exception as exc:
        ...
        while current_url and page_num <= max_pages and len(products) < limit:
            ...
            for attempt in range(1, self.max_retries + 1):
                ...
                try:
                    ...
                    logger.warning(f"Timeout opening Amazon search page (attempt {attempt}): {current_url}: {exc}")
                    if attempt < self.max_retries:
                    ...
                    print(f"Amazon navigation error (attempt {attempt}/{self.max_retries}): {exc}")
                    logger.warning(f"Error opening Amazon search page (attempt {attempt}): {current_url}: {exc}")
